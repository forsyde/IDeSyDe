...
import idesyde.common.legacy.InstrumentedComputationTimes
...
trait MixedRules {
  ...
  def integratePeriodicWorkloadToPartitionedSharedMultiCore(
    ...
    tryCast(decisionModel, classOf[PeriodicWorkloadToPartitionedSharedMultiCore]) { solveds =>
  ...
  def integrateSDFToTiledMultiCore(
    ...
    tryCast(decisionModel, classOf[SDFToTiledMultiCore]) { filtered =>
      val solveds = filtered.filter(m => !m.processMappings.isEmpty && !m.messageMappings.isEmpty)
      for (solved <- solveds; rebuilt = SystemGraph()) yield {
  ...
  def identPeriodicWorkloadToPartitionedSharedMultiCoreWithUtilization(
    ...
    ForSyDeIdentificationUtils.toForSyDe(models) { model =>
      tryCast(identified, classOf[PeriodicWorkloadToPartitionedSharedMultiCore]) { fulls =>
        (
          fulls.map(
            _.copy(maxUtilizations =
              (for (
                pe <- fulls.head.platform.hardware.processingElems;
