...
class DependentWorkloadFixedPrioPropagator(
...
) extends Propagator[IntVar](taskExecutions ++ taskResponses, PropagatorPriority.QUADRATIC, false) {
  ...
  override def getPropagationConditions(vIdx: Int): Int = {
    ...
    // if (0 <= vIdx && vIdx < taskExecutions.length) {
  ...
  //override def compareTo(x: Propagator[IntVar]): Int = -1
