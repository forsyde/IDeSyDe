...
class PanoramaUseCaseWithSolutionSuite extends AnyFunSuite with LoggingMixin {
  ...
  test("PANORAMA case study - can write back model before solution") {
    forSyDeModelHandler.writeModel(model, "scala-tests/models/panorama/input_to_dse.fiodl")
    forSyDeModelHandler.writeModel(model, "scala-tests/models/panorama/input_to_dse.amxmi")
    forSyDeModelHandler.writeModel(model, "scala-tests/models/panorama/input_to_dse_visual.kgt")
  }
