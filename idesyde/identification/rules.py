...
def identify_sdf_app(model: ForSyDeModel, identified: List[DecisionModel]):
    ...
    '''
    result = None
    constructors: Sequence[SDFComb] = [
        c for c in model if isinstance(c, SDFComb)]
    delay_constructors: Sequence[SDFPrefix] = [
        c for c in model if isinstance(c, SDFPrefix)]
    # 1: find the actors
    sdf_actors: List[Vertex] = [
        a for c in constructors for a in model[c] if isinstance(a, Process)]
    # there are garanteed to be the correct vertexes by construction
    sdf_constructors = {
        a: c for a in sdf_actors for c in constructors if a in model[c]}
    sdf_impl = {a: i for a in sdf_actors for i in model[sdf_constructors[a]] if isinstance(
        i, InstrumentedFunction)}
    # 1: find the delays
    sdf_delays: List[Vertex] = [
        a for c in delay_constructors for a in model[c] if isinstance(a, Process)]
        ...
        for t in sdf_actors:
            if s != t:
                try:
                    ...
                    paths: Sequence[Sequence[Vertex]] = [path[1:-1]
    ...
    for (cidx, (s, t)) in enumerate(sdf_channels):
        ...
        for path in sdf_channels[(s, t)]:
            ...
            sdf_topology[cidx, sidx] += int(
                sdf_constructors[s].get_production()[out_port.identifier])
            sdf_topology[cidx, tidx] -= int(
                sdf_constructors[t].get_consumption()[in_port.identifier])
    ...
    if len(null_space) == 1:
        ...
        if schedule != []:
            ...
            result = SDFExecution(sdf_actors=sdf_actors,
                                  sdf_constructors=cast(
                                      Dict[Vertex, Vertex], sdf_constructors),
                                  sdf_impl=cast(
                                      Dict[Vertex, Vertex], sdf_impl),
    ...
    if sdf_exec_sub:
        ...
        if orderings:
            ...
            res = SDFToOrders(sdf_exec_sub=sdf_exec_sub,
                              orderings=orderings, pre_scheduling=pre_scheduling)
    # conditions for fixpoints and partial identification
    if res:
...
def identify_sdf_multi_core(model: ForSyDeModel, identified: List[DecisionModel]):
    ...
    if sdf_orders_sub:
        ...
        connections: Dict[Tuple[Vertex, Vertex],
        ...
        if len(cores) + len(comms) <= len(sdf_orders_sub.orderings):
            ...
            res = SDFToMultiCore(sdf_orders_sub=sdf_orders_sub,
                                 ...
                                 comms=comms,
                                 connections=cast(
                                     Dict[Tuple[Vertex, Vertex], Sequence[Sequence[Vertex]]], connections),
...
def identify_jobs_from_sdf_multicore(model: ForSyDeModel,
    ...
    if sdf_mpsoc_char_sub:
        ...
        procs: List[Sequence[Vertex]] = [[p, o] for (p, o) in itertools.product(cores, orderings)
        ...
        comm_capacity: Dict[CommType, int] = {
            i: max(c.get_slots() if isinstance(
                c, TimeDivisionMultiplexer) else 1 for c in li)
                ...
                for (cidx, (s, t)) in enumerate(sdf_channels):
                    if s == job and t == jjob:
                        for (i, _) in enumerate(sdf_mpsoc_char_sub.sdf_mpsoc_sub.comms):
                            wcct[((j, job), (jj, jjob), i)
                                 ] = sdf_mpsoc_char_sub.token_wcct[cidx, i]
        orderings = sdf_mpsoc_char_sub.sdf_mpsoc_sub.sdf_orders_sub.orderings
        ...
        for (i, actor) in jobs:
            ...
            triggers: Collection[int] = sorted((int(k) for (k, v) in o.get_trigger_time().items()
                                                if v == actor.identifier for o in connected_orderings))
        ...
        comm_channels = {((i1, actor1), (i1, actor2)): sdf_channels[(actor1, actor2)]
                         for ((i1, actor1), (i2, actor2)) in itertools.product(jobs, jobs)
                         if (actor1, actor2) in sdf_channels}
        res = JobScheduling(
            abstracted=cast(
                Sequence[Vertex],
                set(sdf_mpsoc_char_sub.covered_vertexes()) | set(
                    sdf_mpsoc_char_sub.wcet_vertexes)
            ...
            objective_weights=[
                sdf_mpsoc_char_sub.throughput_importance, sdf_mpsoc_char_sub.latency_importance],
...
def identify_jobs_from_multi_sdf(model: ForSyDeModel,
                                 identified: List[DecisionModel]) -> Tuple[bool, Optional[DecisionModel]]:
    sdf_multicore_sub: Optional[SDFToMultiCore] = next(
        (p for p in identified if isinstance(p, SDFToMultiCore)), None)
    # early exit
    ...
    procs: List[Sequence[Vertex]] = [[p, o] for (p, o) in itertools.product(sdf_multicore_sub.cores, orderings)
                                     if model.has_edge(p, o) or model.has_edge(o, p)]
    for (p, o) in zip(cores, orderings):
        if not any(p in li or o in li for li in procs):
            procs.append([p, o])
    ...
    comms: List[Sequence[Vertex]] = [[c, o]
    ...
    for (cidx, comm) in enumerate(comms):
        cap = min((c.get_slots() for c in comm if isinstance(
            c, TimeDivisionMultiplexer)), default=1)
        ckey = comms_key[cidx]
    ...
    for a in sdf_actors:
        for (pidx, proc) in enumerate(procs):
            ...
            core = next(u for u in proc if isinstance(
                u, AbstractProcessingComponent))
            ...
            paths = (path for path in paths if sum(
                1 for u in path if isinstance(u, Process)) == 1)
            # cn only have one core, the one in the loop
            paths = (path for path in paths if sum(
                1 for u in path if isinstance(u, AbstractProcessingComponent)) == 1)
    ...
    comm_channels = {((jsi, js), (jti, jt)): sdf_channels[(js, jt)]
    ...
    res = JobScheduling(abstracted=list(sdf_multicore_sub.covered_vertexes()),
                        ...
                        comm_channels=comm_channels,
                        weak_next=cast(
                            Dict[JobType, Sequence[JobType]], weak_next),
                        strong_next=cast(
                            Dict[JobType, Sequence[JobType]], strong_next),
                        paths=sdf_multicore_sub.connections,
                        pre_mapping=pre_mapping,
                        pre_scheduling=pre_scheduling,
                        goals_vertexes=goals_vertexes,
                        objective_weights=[
                            throughput_importance, latency_importance],
                        procs_capacity=procs_capacity,
                        wcet={},
                        wcct={})
    return (True, res)
...
def identify_instrumentation_in_jobs_from_sdf(model: ForSyDeModel, identified: List[DecisionModel]):
    res = None
    sub_jobs: Optional[JobScheduling] = next(
        (p for p in identified if isinstance(p, JobScheduling)), None)
    sub_sdf: Optional[SDFExecution] = next(
        (p for p in identified if isinstance(p, SDFExecution)), None)
    if sub_jobs and sub_sdf:
        ...
        jobs_instrumented = all(a in sub_sdf.sdf_impl and isinstance(sub_sdf.sdf_impl[a], InstrumentedFunction)
                                for (_, a) in sub_jobs.jobs)
        if jobs_instrumented and len(instrumented_procs) == len(sub_jobs.procs):
            worst_cycles = np.zeros(
                (len(sub_jobs.jobs), len(instrumented_procs)), dtype=np.uint64)
            for ((jidx, j), (pidx, p)) in itertools.product(sub_jobs.jobs, enumerate(instrumented_procs)):
                ...
                worst_cycles[jidx, pidx] += p.get_clock_cycles_per_float_op() * \
                    impl.get_max_float_operations()
                worst_cycles[jidx, pidx] += p.get_clock_cycles_per_integer_op() * \
                    impl.get_max_int_operations()
                worst_cycles[jidx, pidx] += p.get_clock_cycles_per_boolean_op() * \
                    impl.get_max_boolean_operations()
            ...
            while (time_scale * min_clocks) // max_freq == 0:
                time_scale *= 1000
            wcet = ((time_scale * worst_cycles) /
                    frequency_matrix).astype(np.uint64)
            wcct = np.zeros((len(sub_jobs.jobs), len(
                sub_jobs.jobs), len(sub_jobs.comms)), dtype=np.uint64)
            for ((jidx, actorj), (jjidx, actorjj), (cidx,
                                                    comm)) in itertools.product(sub_jobs.jobs, sub_jobs.jobs,
                ...
                if (actorj, actorjj) in sub_sdf.sdf_channels:
                    ...
                    wcct[jidx, jjidx, cidx] = sum(
                        math.ceil(
                            ...
                            float(u.get_max_bandwith_bytes_per_sec())) for u in comm
...
def identify_location_req_in_jobs_from_sdf(model: ForSyDeModel,
    ...
    if sub_jobs and sub_sdf:
        ...
        location_vertexes = [
            li for li in model if isinstance(li, LocationRequirement)]
        located_jobs = {l: [(i, j) for (i, j) in enumerate(
            sub_jobs.jobs) if j in model[l]] for l in location_vertexes}
        located_procs = {
            l: [(i, p) for (i, p) in enumerate(sub_jobs.procs)
                if any(u in model[l] for u in p)]
        ...
        res = sub_jobs.new(abstracted=set(sub_jobs.covered_vertexes()) | set(location_vertexes),
...
def identify_merge_job_scheduling_simple(model: ForSyDeModel,
                                         identified: List[DecisionModel]) -> Tuple[bool, Optional[DecisionModel]]:
    sub_jobs: Collection[JobScheduling] = [
        p for p in identified if isinstance(p, JobScheduling)]
    if len(sub_jobs) <= 1:
        return (False, None)
    covered: Sequence[Vertex] = reduce(
        operator.or_, (set(m.covered_vertexes()) for m in identified), initial=set())
    jobs: Sequence[JobType] = reduce(
        operator.or_, (set(sub.jobs) for sub in sub_jobs), initial=set())
    ...
    paths = {}
    for sub in sub_jobs:
        proc_candidates = [(_hash_sequence(proc), proc) for proc in sub.procs]
        procs += [proc for (key, proc)
                  in proc_candidates if key not in procs_keys]
        procs_keys = procs_keys | set(key for (key, proc) in proc_candidates)
        comm_candidates = [(_hash_sequence(comm), comm) for comm in sub.comms]
        comms += [comm for (key, comm)
                  in comm_candidates if key not in comms_keys]
        comms_keys = comms_keys | set(key for (key, comm) in comm_candidates)
        for (j1, j2) in sub.comm_channels:
            comm_channels[(j1, j2)] = (set(comm_channels[(j1, j2)]) | set(sub.comm_channels[(j1, j2)])
                                       if (j1, j2) in comm_channels else sub.comm_channels[(j2, j2)])
        for (j, actor) in sub.jobs:
            weak_next[(j, actor)] = (set(weak_next[(j, actor)]) | set(sub.weak_next[(j, actor)])
                                     if (j, actor) in weak_next else sub.weak_next[(actor, actor)])
            strong_next[(j, actor)] = (set(strong_next[(j, actor)]) | set(sub.strong_next[(j, actor)])
                                       if (j, actor) in strong_next else sub.strong_next[(actor, actor)])
        for (j1, j2) in s.paths:
            paths[(j1, j2)] = (set(paths[(j1, j2)]) | set(s.paths[(j1, j2)])) if (j1, j2) in paths else s.paths[(j2,
    ...
    goals_vertexes=[]
    objective_weights=[]
    wcet: Dict[Tuple[JobType, ProcType],
               int]=reduce(lambda d1, d2: {k: max(d1[k], d2[k]) if k in d1 and k in d2 else d2[k]
                                             for k in d2}, (s.wcet for s in sub_jobs),
                             initial={})
    wcct: Dict[Tuple[JobType, JobType, ProcType],
               int]=reduce(lambda d1, d2: {k: max(d1[k], d2[k]) if k in d1 and k in d2 else d2[k]
                                             for k in d2}, (s.wcct for s in sub_jobs),
                             initial={})
    location_req: Dict[JobType, Sequence[ProcType]]={}
    res=JobScheduling(abstracted=reduce(operator.or_, (s.abstracted for s in sub_jobs), initial=set()),
                        jobs=jobs,
                        procs=procs,
                        comms=comms,
                        comm_channels=comm_channels,
...
def identify_time_triggered_platform(model: ForSyDeModel,
    ...
    # there must be orderings for both execution and communication
    comms_bandwidth={
        c: c.get_max_bandwith_bytes_per_sec() if isinstance(
            c, InstrumentedCommunicationInterconnect) else 0
        for c in comms
    }
    core_memory={
        p: p.get_max_memory_internal_bytes() if isinstance(
            p, InstrumentedProcessorTile) else 0
        for p in cores
    }
    # TODO: Identify also connected memory elements!

    core_scheduler={
        p: next((o for o in schedulers if o in model[p]), None) for p in cores}
    comm_scheduler={
        p: next((o for o in schedulers if o in model[p]), None) for p in comms}
...
def identify_jobs_sdf_time_trigger_multicore(model: ForSyDeModel, identified: List[DecisionModel]):
    ...
    sdf_app_sub=next(
        (p for p in identified if isinstance(p, SDFExecution)), None)
    time_trig_platform_sub=next(
        (p for p in identified if isinstance(p, TimeTriggeredPlatform)), None)
    all_covered=all(
    ...
    if all(nx.has_path(model, g, a) for g in throughput_vertexes for a in sdf_app_sub.sdf_actors):
        throughput_importance=max(
            (int(v.properties['apriori_importance']) for v in throughput_vertexes), default=0)
    comm_channels={((jsi, js), (jti, jt)): sdf_app_sub.sdf_channels[(js, jt)]
                     for (jsi, js) in jobs for (jti, jt) in jobs if (js, jt) in sdf_app_sub.sdf_channels}
    res=JobScheduling(
        abstracted=set(sdf_app_sub.covered_vertexes()) | set(
            time_trig_platform_sub.covered_vertexes())
        | set(goals_vertexes),
        jobs=jobs,
        comm_channels=comm_channels,
        weak_next=weak_next,
        strong_next=strong_next,
        procs_capacity=[time_trig_platform_sub.core_memory[p]
                        for p in time_trig_platform_sub.core_scheduler],
        comm_capacity=[time_trig_platform_sub.comms_bandwidth[p]
                       for p in time_trig_platform_sub.comm_scheduler],
        goals_vertexes=goals_vertexes,
        objective_weights=[throughput_importance, latency_importance],
        procs=[[p, o]
               for (p, o) in time_trig_platform_sub.core_scheduler.items()],
