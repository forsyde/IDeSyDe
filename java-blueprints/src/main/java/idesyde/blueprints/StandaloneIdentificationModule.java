        ...
        try (var server = Javalin.create()) {
            server.post(
                            ...
                            ctx -> {
                                if (ctx.isMultipart()) {
                                    ctx
                                            ...
                                            .forEach(message -> {
                                                try (var content = message.content()) {
                                                    var msg = new String(content.readAllBytes(), StandardCharsets.UTF_8);
                                                    DesignModelMessage.fromJsonString(msg)
                    ...
                    .get(
                            "/identify",
                            ctx -> {
                                var iteration = ctx.queryParamMap().containsKey("iteration") ? Integer.parseInt(ctx.queryParam("iteration")) : 0;
                                var result = identificationStep(
                                        iteration,
                                        designModels,
                                        decisionModels
                                );
                                var newIdentified = new HashSet<>(result.identified());
                                newIdentified.removeAll(decisionModels);
                                decisionModels.addAll(newIdentified);
                                new IdentificationResultMessage(
                                        newIdentified.stream().map(x -> new DecisionModelMessage(x.header(), x.bodyAsText())).collect(Collectors.toSet()),
                                        result.errors()
                    ...
                    .get(
                            ...
                            ctx -> {
                                ...
                                ctx.result(objectMapper.writeValueAsString(integrated.stream().map(x -> DesignModelMessage.from(x)).collect(Collectors.toList())));
                            }
                    )
                    .exception(
                            Exception.class,
