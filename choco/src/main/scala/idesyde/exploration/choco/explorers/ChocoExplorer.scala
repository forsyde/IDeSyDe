...
class ChocoExplorer() extends ForSyDeIOExplorer:
  ...
  private def getLinearizedObj(forSyDeDecisionModel: ChocoCPForSyDeDecisionModel): IntVar = {
    val normalizedObjs = forSyDeDecisionModel.modelMinimizationObjectives.map(o => 
      ...
      val scaled = forSyDeDecisionModel.chocoModel.intVar(s"scaled(${o.getName()})",
        0,
        100,
        true
      )
    ...
    val scalarizedObj = forSyDeDecisionModel.chocoModel.intVar("scalarObj", 
      0,
      10000,
      true
    )
    forSyDeDecisionModel.chocoModel.scalar(normalizedObjs, normalizedObjs.map(o => 100 / normalizedObjs.size), "=", scalarizedObj).post()
    scalarizedObj
  }
  ...
  def exploreForSyDe(forSyDeDecisionModel: ForSyDeDecisionModel)(using
    ...
    case chocoCpModel: ChocoCPForSyDeDecisionModel =>
      ...
      lazy val paretoMaximizer = ParetoMaximizer(chocoCpModel.modelMinimizationObjectives.map(o => chocoCpModel.chocoModel.intMinusView(o)))
      ...
      if (isOptimization) {
        ...
        if (chocoCpModel.modelMinimizationObjectives.size == 1) {
          chocoCpModel.chocoModel.setObjective(true, chocoCpModel.chocoModel.intMinusView(chocoCpModel.modelMinimizationObjectives.head))
        }
          ...
          if (isOptimization) {
            ...
            if (lastParetoFrontSize != paretoMaximizer.getParetoFront().size()) {
              ...
              paretoMaximizer.getParetoFront().forEach(s => {
                val isDominator = chocoCpModel.modelMinimizationObjectives.zipWithIndex.forall((o, i) => {
                  s.getIntVal(o) < lastParetoFrontValues(i)
                })
                if (isDominator) {
                  lastParetoFrontValues = chocoCpModel.modelMinimizationObjectives.map(o => s.getIntVal(o))
                  paretoFrontChanged = true
                }
              })
            }
