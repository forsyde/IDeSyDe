...
trait SDFChocoRecomputeMethodsMixin {
    ...
    /** whether channel c sends data at slot i, from tile src to tile dst at slot i+1. order is (i)(c)(p)(pp) */
    ...
    protected lazy val startTimes:  Array[Array[Int]] = schedulers.map(_ => slots.map(_ => 0).toArray).toArray
    protected lazy val finishTimes: Array[Array[Int]] = schedulers.map(_ => slots.map(_ => 0).toArray).toArray
    ...
    protected lazy val firingVector: Array[SparseVector[Int]] = slots.map(slot => SparseVector.zeros[Int](actors.size)).toArray

    def sendVec(slot: Int)(srcSched: Int)(dstSched: Int): SparseVector[Int] = if (slot > 0) {
      - (consMat * firingVectorPerCore(slot)(dstSched) + prodMat * firingVectorPerCore(slot - 1)(srcSched))  // + tokensBefore(slot - 1)
    } else {
      SparseVector.zeros[Int](channels.size)
    }
    
    protected lazy val firingVectorPerCore: Array[Array[SparseVector[Int]]] = slots.map(i => schedulers.map(p => SparseVector.zeros[Int](actors.size)).toArray).toArray
    ...
    def recomputeFiringVectors(): Unit = {
      for (
        ...
        for (p <- schedulers; if firingVectorPerCore(slot)(p)(a) != 0) { firingVectorPerCore(slot)(p)(a) = 0 }
        for (p <- schedulers) { 
    ...
    def recomputeTokensAndTime(): Unit = {
      ...
      for (
        ...
        for (p <- schedulers) {
          ...
          for (pp <- schedulers; if pp != p) {
            commRecvTime = 0
            val sendVecSaved = sendVec(i)(pp)(p)
            for (c <- channels) {
              commRecvTime = Math.max(commRecvTime, channelsTravelTime(c)(pp)(p).getUB() * sendVecSaved(c))
            }
