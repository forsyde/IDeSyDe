...
import idesyde.common.legacy.RuntimesAndProcessors

trait PlatformRules {

  def identRuntimesAndProcessors(
      models: Set[DesignModel],
      identified: Set[DecisionModel]
  ): (Set[RuntimesAndProcessors], Set[String]) = {
    ForSyDeIdentificationUtils.toForSyDe(models){ model =>
      ...
      val hostedRuntimes = runtimeElements
        .filter(s => s.host().isPresent())
      val hostMap = hostedRuntimes
        .map(s => s.getIdentifier() -> s.host().get().getIdentifier())
        .toMap
      val affinityMap = processingElements
        .flatMap(p =>
          runtimeElements
            .find(s => s.managed().contains(p))
            .map(s => p.getIdentifier() -> s.getIdentifier())
        )
        .toMap
      if (affinityMap.size <= 0) {
        errors += "identRuntimesAndProcessors: no processing elements being managed"
      }
      val m =
        if (processingElements.length > 0) {
          Set(
            RuntimesAndProcessors(
              hostedRuntimes.map(_.getIdentifier()).toSet,
              processingElements.map(_.getIdentifier()).toSet,
              hostMap,
              affinityMap,
              hostedRuntimes.map(_.getIdentifier()).toSet,
              hostedRuntimes
                .filter(ForSyDeHierarchy.FixedPriorityScheduledRuntime.tryView(_).isPresent())
                .map(_.getIdentifier())
                .toSet,
              hostedRuntimes
                .filter(
              ...
              Set(),
              hostedRuntimes
                .filter(ForSyDeHierarchy.SuperLoopRuntime.tryView(_).isPresent())
                .map(_.getIdentifier())
                .toSet
            )
          )
