package idesyde.identification.forsyde.rules.reactor
...
final case class ReactorMinusIdentificationRule(executor: ThreadPoolExecutor)
  ...
  def identifyFromForSyDe(model: ForSyDeSystemGraph, identified: scala.collection.Iterable[DecisionModel]) = {
    ...
    if (elements.length > 0 && ReactorMinusIdentificationRule.canIdentify(model, identified)) {
      ...
      val timers = elements
        ...
        .toArray
      val found = ReactorMinusApplication(
      ...
      scribe.debug(
        "Conforming Reactor- model found with:" +
          s"${found.pureReactions.size} pure reaction(s), " +
          s"${found.periodicReactions.size} periodic reaction(s), " +
          s"${found.reactors.size} reactor(s), " +
          s"${found.channels.size} channel(s)," +
          s"${found.unambigousEndToEndReactions.size} trivial chain(s) and " +
          s"hyperperiod of ${found.hyperPeriod}"
      )
  ...
  def computePeriodFunction(
      ...
      reactions: Array[LinguaFrancaReaction]
  ): Map[LinguaFrancaReaction, Rational] =
    timers
      .flatMap(t =>
        reactions
      ...
      .map((r, t) =>
        r -> Rational(
          t.getPeriodNumeratorInSec.toInt,
          t.getPeriodDenominatorInSec.toInt
        )
      )
      .toMap
