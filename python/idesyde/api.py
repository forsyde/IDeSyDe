...
class DeSyDeR(object):
    ...
    def __init__(self):
        self.standard_problems = set(c for c in DecisionModel.__subclasses__())
        self.standard_explorers = set(s() for s in Explorer.__subclasses__())

    async def identify_problems(self,
                                model: ForSyDeModel,
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=concurrent_idents) as executor:
            while len(candidates) > 0 and iterations < max_iterations:
                ...
                # made the trial
                futures = {c: executor.submit(c.identify, model, identified) for c in candidates}
    ...
    async def explore_problems(
        self, model: ForSyDeModel, problems: Set[DecisionModel],
        explorers: Set[Explorer] = set()) -> Optional[ForSyDeModel]:
