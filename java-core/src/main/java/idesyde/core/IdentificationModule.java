...
/**
 * The trait/interface for an identification module that provides the
 * identification and
 * integration rules required to power the design space identification process
 * [1].
 *
 * This trait extends [[idesyde.core.IdentificationLibrary]] to push further the
 * modularization of
 * the DSI methodology. In essence, this trait transforms an
 * [[idesyde.core.IdentificationLibrary]]
 * into an independent callable library, which can be orchestrated externally.
...
public interface IdentificationModule {
    ...
    // default Optional<DesignModel> inputsToDesignModel(Path p) { return
    // Optional.empty(); }

    /**
     * decoders used to reconstruct decision models from headers.
     *
     * Ideally, these functions are able to produce a decision model from the
     * headers read during a
     * call of this module.
     *
     * @return
    ...
    // default Optional<Path> designModelToOutput(DesignModel m, Path p) {return
    // Optional.empty(); }
