...
class SimpleListSchedulingDecisionStrategy(val schedulers: Array[Int], val taskExecutions: Array[IntVar], val responseTimes: Array[IntVar]) extends
  AbstractStrategy[IntVar]((taskExecutions ++ responseTimes):_*) {
    ...
    def getDecision(): Decision[IntVar] = {
        ...
        val maxRs = schedulers.zipWithIndex.map((s, j) => {
          responseTimes.zipWithIndex.map((t, i) => {
            if (taskExecutions(i).contains(j)) responseTimes(i).getUB
            else 0
          }).max
        })
        taskExecutions.find(!_.isInstantiated).map(next => {
