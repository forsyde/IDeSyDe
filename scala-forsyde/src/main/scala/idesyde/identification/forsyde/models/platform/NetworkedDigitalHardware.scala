...
final case class NetworkedDigitalHardware(
  ...
  lazy val communicationModuleBandWidthBitPerSec = communicationElems.zipWithIndex.map((ce, i) =>
    InstrumentedCommunicationModule
      .safeCast(ce)
      .map(insce =>
        Rational.one * insce.getFlitSizeInBits.toInt * insce.getMaxConcurrentFlits.toInt * insce.getOperatingFrequencyInHertz.toInt
      ...
      (e) => {
        ...
        ceOpt
          .flatMap(ce => {
            ...
            val fraction: Rational = RoundRobinCommunicationModule
              .safeCast(ce)
              .map(rrce => {
                Rational(
                  rrce.getAllocatedWeights.getOrDefault(other.getIdentifier, 1).intValue(),
                  rrce.getTotalWeights.intValue()
                )
              })
              .orElse(Rational.one)
  ...
  val allocatedBandwidthFraction
      : Map[(GenericCommunicationModule, GenericProcessingModule), Rational] =
    (for (
      ...
      case Some(rr) =>
        (ce, pe) -> Rational(
          ...
          rr.getAllocatedWeights.getOrDefault(pe.getViewedVertex.getIdentifier, 0).intValue(),
          rr.getTotalWeights.intValue()
        )
      case _ => (ce, pe) -> Rational(0)
    }).toMap
  ...
  //         case _ => Option(0L)
  //     )
  //   if (slidenSum.forall(_.isDefined)) then Option(slidenSum.map(_.get).map(Rational(1L, _)).sum)
  //   //Option(Rational(1, headBW.get).add(pathSum).add(Rational(1, tailBW.get)))
  //   else Option.empty
  ...
  //   val pathSum = slidenSum.map(_.get).map(Rational(1L, _)).sum
  //   Option(Rational(1, headBW.get).add(pathSum).add(Rational(1, tailBW.get)))
  // else Option.empty
    ...
    platformElements.zipWithIndex.map((src, i) => {
      platformElements.zipWithIndex.map((dst, j) => {
        if (i != j && paths.getPath(i, j) != null) {
          Rational(maxWeight) - Rational(paths.getPathWeight(i, j))
        } else
  ...
  lazy val maxTraversalTimePerBit: Array[Array[Rational]] = {
    ...
    platformElements.zipWithIndex.map((src, i) => {
      platformElements.zipWithIndex.map((dst, j) => {
        if (i != j && paths.getPath(i, j) != null) {
          Rational(paths.getPathWeight(i, j))
        } else
          Rational.zero - Rational.one
        // val commPath = inclusiveDirectCommPaths(src)(dst)
        // // for the first element
        ...
        //   Option(slidenSum.map(_.get).map(Rational(1L, _)).sum)
        // //Option(Rational(1, headBW.get).add(pathSum).add(Rational(1, tailBW.get)))
        // else Option.empty
      })
    })
