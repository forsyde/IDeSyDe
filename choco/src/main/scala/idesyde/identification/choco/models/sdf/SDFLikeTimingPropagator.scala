        ...
        wfor(0, _ < numSchedulers, _ + 1) {p => 
          ...
          wfor(0, _ < numSchedulers, _ + 1) {pp => 
            if (pp != p && slotOccupied(p)(i) && slotOccupied(pp)(i - 1)) {
              var commRecvTime = 0
              val diffAtP = SDFChocoRecomputeMethods.diffTokenVec(consMat)(prodMat)(firingVectorPerCore(i)(p))(firingVectorPerCore(i - 1)(p))
              val diffAtPP = SDFChocoRecomputeMethods.diffTokenVec(consMat)(prodMat)(firingVectorPerCore(i)(pp))(firingVectorPerCore(i - 1)(pp))
              wfor(0, _ < numChannels, _ + 1) {c => 
                if (diffAtP(c) < 0 && diffAtPP(c) >= -diffAtP(c)) {
                  commRecvTime = Math.max(commRecvTime, -channelsTravelTime(c)(pp)(p).getLB() * diffAtP(c))
                }
              }
