...
final case class ReactorMinusAppMapAndSched(
    ...
    val wcetFunction: Map[(LinguaFrancaReaction, GenericProcessingModule), Rational],
    val utilityFunction: Map[(LinguaFrancaReaction, GenericProcessingModule), Rational]
  ...
  lazy val jobUtilityFunction: Map[(ReactionJob, GenericProcessingModule), Rational] =
    reactorMinus.jobGraph.jobs
      .flatMap(j =>
